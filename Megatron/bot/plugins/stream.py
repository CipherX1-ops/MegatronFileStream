        ...
        if file and "GiB" in str(file_size) and not m.from_user.id in Var.PRO_USERS:
            ...
            if is_spam:
                ...
                if u.status == "kicked" or u.status == "banned":
                    ...
                    stream_link = f"{Var.URL}{log_msg.message_id}/{get_name(m)}?hash={get_hash(log_msg)}"
...
async def channel_receive_handler(bot, broadcast):
    ...
    try:
        ...
        stream_link = f"{Var.URL}{log_msg.message_id}/{get_name(broadcast)}?hash={get_hash(log_msg)}"
